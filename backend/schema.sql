...
DROP TABLE IF EXISTS chats_participantes;
DROP TABLE IF EXISTS chats;
DROP TABLE IF EXISTS votos_usuarios;
...
CREATE TABLE IF NOT EXISTS miembros_comunidad (
    ...
    UNIQUE KEY unique_miembro (usuario_id, comunidad_id),
...
CREATE TABLE IF NOT EXISTS votos_usuarios (
    ...
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_voto (usuario_id, publicacion_id),
    FOREIGN KEY (usuario_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (publicacion_id) REFERENCES publicaciones(id) ON DELETE CASCADE
);
...
ALTER TABLE publicaciones
    ADD CONSTRAINT fk_publicaciones_media_asset
    FOREIGN KEY (media_asset_id) REFERENCES media_assets(id) ON DELETE SET NULL;

ALTER TABLE mensajes_chat
    ADD CONSTRAINT fk_mensajes_media_asset
    FOREIGN KEY (media_asset_id) REFERENCES media_assets(id) ON DELETE SET NULL;
